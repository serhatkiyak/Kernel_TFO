...
do
	...
	do	
		mkdir $a/$i
		(time ./wget -P $a -e robots=off -4 -H -p -k $url -o $a/$i/tcp_log.txt) > $a/$i/tcp_time.txt 2>&1
	done
	sleep 1
	kill -SIGTERM $pid
	...
	for i in {1..6..1}
	do	
		(time ./wget -P $a -e robots=off -4 -H -p -k $url -o $a/$i/tfo_log.txt) > $a/$i/tfo_time.txt 2>&1
	done
	sleep 1
	kill -SIGTERM $pid
